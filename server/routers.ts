...
const subjectInput = z.object({ name: z.string().trim().min(2).max(120), teacherName: z.string().trim().max(180).optional(), color: z.string().max(24).default("teal") });
...
const updateSubjectInput = z.object({ id: z.number().int().positive(), name: z.string().trim().min(2).max(120), teacherName: z.string().trim().max(180).optional(), color: z.string().max(24), sortOrder: z.number().int().min(0).max(999) });
