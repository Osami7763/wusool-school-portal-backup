...
let defaultSubjectsPromise: Promise<void> | null = null;
 export async function ensureDefaultSubjects() { if (defaultSubjectsPromise) return defaultSubjectsPromise; defaultSubjectsPromise = (async () => { const db = await getDb(); if (!db) return; const existing = await db.select({ id: subjects.id, name: subjects.name }).from(subjects); const oldQuran = existing.find(item => item.name === "القرآن الكريم وتفسيره"); if (oldQuran) await db.update(subjects).set({ name: "القرآن الكريم وتفسيره (أو التفسير)" }).where(eq(subjects.id, oldQuran.id)); const names = new Set(existing.map(item => item.name)); names.add("القرآن الكريم وتفسيره (أو التفسير)"); for (let index = 0; index < defaultSubjects.length; index += 1) { const name = defaultSubjects[index]; if (!names.has(name)) { try { await db.insert(subjects).values({ name, teacherName: null, color: ["teal", "blue", "violet", "amber", "rose"][index % 5], sortOrder: index }); } catch (error) { console.error("Default subject insert failed", { name, error: String(error) }); const found = await db.select({ id: subjects.id }).from(subjects).where(eq(subjects.name, name)).limit(1); if (!found[0]) throw error; } names.add(name); } } })().catch(error => { defaultSubjectsPromise = null; throw error; }); return defaultSubjectsPromise; }
...
export async function createSubject(input: { name: string; teacherName?: string; color: string }) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const r = await db.insert(subjects).values({ ...input, teacherName: input.teacherName || null }); return db.select().from(subjects).where(eq(subjects.id, Number(r[0].insertId))).limit(1).then(x => x[0]); }
