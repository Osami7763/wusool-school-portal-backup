...
const defaultSubjects = ["الرياضيات 1-1", "الكفايات اللغوية 1-1 (اللغة العربية)", "الكيمياء 1", "الأحياء 1", "القرآن الكريم وتفسيره", "اللغة الإنجليزية 1 (Mega Goal)", "التقنية الرقمية 1", "التفكير الناقد"];
export async function ensureDefaultSubjects() { const db = await getDb(); if (!db) return; const existing = await db.select({ name: subjects.name }).from(subjects); const names = new Set(existing.map(item => item.name)); for (let index = 0; index < defaultSubjects.length; index += 1) { const name = defaultSubjects[index]; if (!names.has(name)) await db.insert(subjects).values({ name, teacherName: null, color: ["teal", "blue", "violet", "amber", "rose"][index % 5], sortOrder: index }); } }
