...
export async function createSubject(input: { name: string; color: string }) { const db = await getDb(); if (!db) throw new Error("Database is not available"); const r = await db.insert(subjects).values(input); return db.select().from(subjects).where(eq(subjects.id, Number(r[0].insertId))).limit(1).then(x => x[0]); }
export async function updateSubject(id: number, input: { name: string; color: string; sortOrder?: number }) { const db = await getDb(); if (!db) throw new Error("Database is not available"); await db.update(subjects).set(input).where(eq(subjects.id, id)); return { success: true as const }; }
